  ...
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      context 'with profiles' do
        ...
        it {
          is_expected.to contain_exec('tuned-adm_profile').that_requires('Class[tuned::config]').with(
        ...
        it { is_expected.to contain_file("#{profiles_path}/hpc").with_ensure('directory') }
        it { is_expected.to contain_file("#{profiles_path}/basic").with_ensure('directory') }

        it { is_expected.to contain_file("#{profiles_path}/hpc/tuned.conf").with_content(%r{^\[main\]\ninclude=latency-performance$}) }
        it { is_expected.to contain_file("#{profiles_path}/hpc/tuned.conf").with_content(%r{^\[sysctl\]\nnet.ipv4.tcp_fastopen=3$}) }
        it { is_expected.to contain_file("#{profiles_path}/basic/tuned.conf").with_content(%r{^\[main\]\ninclude=balanced$}) }
      end
      ...
      context 'with operators' do
        ...
        profiles_path = tuned_profiles_path(os_facts)
        it { is_expected.to contain_file("#{profiles_path}/io").with_ensure('directory') }

        it { is_expected.to contain_file("#{profiles_path}/io/tuned.conf").with_content(%r{^\[disk\]\nreadahead=>2048$}) }
      end
    end
  end
end
